...
public class ResultAdapter extends BaseAdapter {
	...
	public ResultAdapter(Context context, Poll poll) {
		...
		DateFormat timeFormat =
			    android.text.format.DateFormat.getTimeFormat(context.getApplicationContext());
		...
		data.add(dateFormat.format(startDate) + " " + timeFormat.format(startDate));
		data.add(new Separator(context.getResources().getString(R.string.number_voters)));
		data.add(numberParticipants + "");
		data.add(new Separator(context.getResources().getString(R.string.number_cast_votes)));
		data.add(numberCastVotes + "");
		data.add(new Separator(context.getResources().getString(R.string.participation)));
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (this.getItem(position) instanceof Poll){
			...
			view = inflater.inflate(R.layout.list_item_option_result, parent, false);
			TextView tvOption = (TextView)view.findViewById(R.id.textview_content);
			TextView tvNumberVotes = (TextView)view.findViewById(R.id.textview_number_votes);
			TextView tvPercentage = (TextView)view.findViewById(R.id.textview_percentage);
			
			tvOption.setText(option.getText());
			if (option.getVotes() == 1){
				tvNumberVotes.setText(option.getVotes() + " " + context.getResources().getString(R.string.vote_singular));
			}
			else {
				tvNumberVotes.setText(option.getVotes() + " " + context.getResources().getString(R.string.vote_plural));
			}
